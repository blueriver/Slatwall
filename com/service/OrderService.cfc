						...
						if(len(orderRequirementsList)) {
							...
							if(!paymentsProcessed) {
								...
								for(var i=1; i<=arrayLen(order.getOrderFulfillments()); i++) {
									...
									if(order.getOrderFulfillments()[i].getFulfillmentMethodType() == "auto" && (order.getOrderFulfillments()[i].setting('fulfillmentMethodAutoMinReceivedPercentage') / 100) >= (order.getPaymentAmountReceivedTotal()/order.getTotal()) ) {
