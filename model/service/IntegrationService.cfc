		...
		for(var i=1; i<= arrayLen(dirList); i++) {
			...
			if(fileInfo.type == "directory" && fileExists("#fileInfo.path#/Integration.cfc") ) {
				...
				if(structKeyExists(integrationMeta, "Implements") && structKeyExists(integrationMeta.implements, "Slatwall.integrationServices.IntegrationInterface")) {
					...
					if(isNull(integration.setFW1ActiveFlag()) || (integration.getFW1ActiveFlag() && !integration.getFW1ReadyFlag())) {
						integration.setFW1ActiveFlag(0);
					}
					if(isNull(integration.setPaymentActiveFlag()) || (integration.getPaymentActiveFlag() && !integration.getPaymentReadyFlag())) {
						integration.setPaymentActiveFlag(0);
					}
					if(isNull(integration.setShippingActiveFlag()) || (integration.getShippingActiveFlag() && !integration.getShippingReadyFlag())) {
